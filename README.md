...
1. Main code contained in `main.c`
    ...
    - There are two interrupts that are used: one is used to react to change of orientation from accelerometer and the other to get the device to and from sleep
...
4. Software drivers:
    ...
    - Debug command line (Code taken from the Elicia White MES Week 5 assignment and adapted to match the requirements of the system):
        ...
        - Logging and mocking procedure for storm prediction algorithm (this part is not in the main branch as is needs testing)
        - Simulating the storm arrival
...
## [Video Demo](https://youtu.be/wZIftxVA01M)
